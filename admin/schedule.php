    ...
    if ($action == 'add-session') {
        ...
        echo '
            ...
            . '</td>
                            ...
                            <td class="label-td" colspan="2">
                                <label for="docid" class="form-label">Select Hospital: </label>
                            </td>
                        </tr>
                            <tr>
                                <td class="label-td" colspan="2">
                                    <select name="hospital_id" id="" class="box" >
                                    <option value="" disabled selected hidden>Choose Hospital from the list</option><br/>';

                                    $list11 = $database->query("SELECT * FROM `hospital` ORDER BY name ASC;");
                            ...
                            <tr>
                                <td class="label-td" colspan="2">
                                    <input type="number" name="fee" class="input-text" min="0"  placeholder="Price" required><br>
                                </td>
                            </tr>
                            <tr>
                                <td class="label-td" colspan="2">
        ...
        $sqlmain = "select schedule.scheduleid,schedule.hospital_id,schedule.title,doctor.docname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join doctor on schedule.docid=doctor.docid where  schedule.scheduleid=$id";
        $result = $database->query($sqlmain);
        $row = $result->fetch_assoc();
        $docname = $row["docname"];
        $scheduleid = $row["scheduleid"];
